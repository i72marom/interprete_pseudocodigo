...
%type <st> stmt asgn print print_string read read_string if block do_while for for_block while while_block do_while do_while_block lugar borrar asignacion_unary case case_block
...
%token PRINT PRINT_STRING READ READ_STRING THEN IF ELSE END_IF WHILE DO REPEAT UNTIL CASE VALUE DEFAULT END_CASE BORRAR LUGAR  

/* NEW in example 17 */
%token LETFCURLYBRACKET RIGHTCURLYBRACKET

/* NEW in example 7 */
%right ASSIGNMENT AUMENTO_IGUAL DISMINUCION_IGUAL
...
%left PLUS MINUS AUMENTO DISMINUCION
...
while_block: DO stmtlist END_WHILE
		{
			$$ = new lp::BlockStmt($2);
		}
do_while_block: REPEAT stmtlist UNTIL
		{
			$$ = new lp::BlockStmt($2);
		}
for_block : DO stmtlist END_FOR
		{
			$$ = new lp::BlockStmt($2);
		}
...
asignacion_unary:
		  VARIABLE AUMENTO
		{
			$$ = new lp::AssignmentStmt($1, new lp::PlusNode($1, new lp::NumberNode(1)));
		}
		| VARIABLE DISMINUCION
		{
			$$ = new lp::AssignmentStmt($1, new lp::MinusNode($1, new lp::NumberNode(1)));
		}
